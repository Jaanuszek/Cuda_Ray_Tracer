...
namespace renderKernelFunctions {
    __device__ color ray_color(const ray& r, hittable_list* world, curandState* r_state)
    {
        ray cur_ray = r;
        ...
        for (int i = 0; i < 50; i++) {
            hit_record rec;
            printf("====== ray_color2");
            //printf("======= ray_color world size %d", (world)->list_size);
            if (world)
            {
                printf("world exits!");
            }
    ...
    __global__ void render_framebuffer(vec3* d_fb, hittable_list* d_world, camera_params camParams, curandState *rand_state)
    {
...
camera::camera()
{
    ...
    renderKernelFunctions::init_rand_state << <gridSize, blockSize >> > (h_render_params->rand_state, image_width, image_height);
...
void camera::render() // moze to world powinno sie tworzyc poza klasa ( w mainie)
{
    ...
    vec3* d_fb = h_render_params->fb;
    hittable_list* d_world = h_render_params->world;
    curandState* d_rand_state = h_render_params->rand_state;
